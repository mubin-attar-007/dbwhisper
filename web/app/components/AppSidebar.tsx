...
const NAV: { label: string; icon: IconName; active?: boolean; soon?: boolean }[] = [
  { label: "Console", icon: "spark", active: true },
  { label: "Databases", icon: "database", soon: true },
  { label: "Saved / Verified", icon: "star", soon: true },
    ...
    <aside className="hidden w-64 shrink-0 flex-col border-r border-slate-800/80 bg-slate-950/50 md:flex">
        ...
        {NAV.map((item) => (
          <div
            ...
            className={`flex items-center gap-3 rounded-md px-3 py-2 text-sm ${
              item.active ? "bg-indigo-950/40 text-indigo-200" : "text-slate-400"
            }`}
          >
            <Icon name={item.icon} className="h-4 w-4 shrink-0" />
            <span className="flex-1 truncate">{item.label}</span>
            {item.soon && (
              <span className="rounded-full border border-slate-700 px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-slate-500">
                soon
              </span>
            )}
          </div>
      ...
      <div className="mt-4 flex min-h-0 flex-1 flex-col px-2">
        <div className="flex items-center justify-between px-1 pb-1">
          ...
          {history.length > 0 && (
            <button
              ...
              className="rounded text-xs text-slate-500 transition hover:text-slate-300 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-indigo-400"
            >
              Clear
            </button>
          )}
        ...
        <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto">
          {history.length === 0 ? (
            <p className="px-1 py-2 text-xs text-slate-600">Your questions will appear here.</p>
          ) : (
              ...
              {history.map((h, i) => (
                <li key={`${h.ts}-${i}`}>
                  ...
                  >
                    ...
                    <span className="shrink-0 text-[10px] text-slate-600">{timeAgo(h.ts)}</span>
                  </button>
                </li>
              ))}
            </ul>
